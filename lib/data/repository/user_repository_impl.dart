...
class UserRepositoryImpl implements UserRepository {
  ...
  @override
  Future<(AppException?, void)> setMasterPin(
      {required int id, required int pin}) async {
    try {
      ...
      final hashedPin =
          SecureHash.generateHashWithSalt(pin.toString(), user.salt!);
      await _db.update(
        'users',
        {
  ...
  Future<(AppException?, void)> setLoggedInUser({required int id}) async {
    try {
      await _localStorage.write(
          key: AppConstant.localUserId, value: id.toString());
      return (null, null);
    } catch (e) {
      return (e.toAppException(), null);
    }
